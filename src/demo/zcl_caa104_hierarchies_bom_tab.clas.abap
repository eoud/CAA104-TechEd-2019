class zcl_caa104_hierarchies_bom_tab definition
  public
  final
  create public .
  ...
  protected section.
  private section.
    methods test_hierarchy_bom_abap_itab importing out type ref to if_oo_adt_classrun_out.
    methods setup.
ENDCLASS.



CLASS ZCL_CAA104_HIERARCHIES_BOM_TAB IMPLEMENTATION.


  method if_oo_adt_classrun~main.
    setup( ).
    test_hierarchy_bom_abap_itab( out ).
  endmethod.
  ...
  method test_hierarchy_bom_abap_itab.

    select * from zcaa104_bom into table @data(bom_source).

    with +demo_asql_bom as ( select from @bom_source as t fields material, parent, quantity )
