CLASS zcl_caa104_windowing_lead DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_caa104_windowing_lead IMPLEMENTATION.
  METHOD special_window_functions.
    ...
    WITH +ctgry AS ( SELECT FROM Z_CAA104_SALES_ORDER_ITEM AS item
         ...
         +rank AS ( SELECT FROM +ctgry
                        ...
                        coalesce( lead( sum  ) OVER( PARTITION BY sales_order_nr ORDER BY sum DESCENDING ), 0 ) as lead,
                        sum - lead( sum ,1, sum ) OVER( PARTITION BY sales_order_nr ORDER BY sum DESCENDING ) as difference_to_succeeding )
    SELECT FROM +rank
      FIELDS sales_order_nr, item_category, rank, total_price, percentage, lead, difference_to_succeeding,
      sum( difference_to_succeeding ) over( partition by sales_order_nr order by rank descending ) as difference_to_last
      ORDER BY rank ascending
      INTO TABLE @DATA(result).
    loop at result assigning field-symbol(<wa>).
      <wa>-difference_to_last = round( val = <wa>-difference_to_last dec = 0 ).
    endloop.
    out->write( result ).
  ENDMETHOD.
